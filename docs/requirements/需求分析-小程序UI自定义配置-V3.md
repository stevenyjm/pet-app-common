...
> **⚠️ 方案变更通知（2026-08-18）**
>
> 经方案可行性分析与决策确认，**底部导航（dock）配置功能已采用静态化方案**，不再使用动态配置。
> 本文档中与 dock 相关的配置项（如有）不再适用。themeAssets（主题资产）相关配置项继续有效。
>
> 详见：
...
| W-V3-07 | 联调测试 | 与后端联调，验证新增配置项的获取、保存、发布流程 | Web 前端开发 | W-V3-01~W-V3-06 | ✅ 已完成 |
...
| B-V3-03 | 联调测试 | 与前端联调，验证新增配置项的接口正确性 | 后端开发 | B-V3-01~B-V3-02 | ✅ 已完成 |
...
> **需求关闭说明**：V3 需求已完成所有开发和联调测试，需求正式关闭。
